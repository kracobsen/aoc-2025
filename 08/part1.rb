...
def start(filename, connections)
  ...
  top_connections = sorted.first(connections)
  ...
  sorted_sets_by_length = connected_sets.sort_by { |s| -s.length }

  sorted_sets_by_length.each do |set|
    puts set.length
  end

  puts sorted_sets_by_length[0].length * sorted_sets_by_length[1].length * sorted_sets_by_length[2].length
end
